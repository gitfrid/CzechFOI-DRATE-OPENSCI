...
Final version: fixed Pandas length mismatch via merge by rel_bin, programmatic reference bin, descriptive framing

Supports staggered eligibility across age cohorts (Czech rollout phases).
This is a descriptive analysis only – not causal inference on vaccine effects.

Author: Grok (finalized 2026-02-05)
Version: 1.8 – GitHub-ready, error-fixed per Albert's critique
"""
...
# Configuration – full population run (0-112)
@dataclass
class Config:
    input_path: Path = Path(r"C:\CzechFOI-DRATE-OPENSCI\Terra\Vesely_106_202403141131.csv")
    out_dir: Path = Path(r"C:\github\CzechFOI-DRATE-OPENSCI\Plot Results\TDR_DiT")
    ...
    lag_min: int = -60
    lag_max: int = 120
    age_bins: tuple = (0, 113)  # Full population (0–112)
    bin_size_days: int = 7      # Weekly bins for memory safety
    ref_bin_days: int = -14     # Target reference bin (pre-eligibility)
...
COHORT_ELIGIBILITY = {
    ...
    (18, 60): pd.Timestamp("2021-06-01"),   # 18–59 (approx)
    (12, 18): pd.Timestamp("2021-09-01"),   # Adolescents (approx)
    (5, 12): pd.Timestamp("2021-12-01"),    # Children (approx)
    (0, 5): pd.Timestamp("2022-01-01"),     # Young children (low/no early vax)
}

# Utilities
def write_run_info(cfg: Config, row_count: int):
    info = {
...
# TDR-DiT: Aggregated Event-Study Poisson GLM
def tdr_dit_analysis(df: pd.DataFrame, cfg: Config = CFG):
    log.info("Running descriptive event-time hazard study")

    # Ensure eligibility varies by cohort
    df['eligibility_date'] = pd.to_datetime(df['eligibility_date'])

    # Compute rel_day for censor and death (per subject)
    df['censor_rel_day'] = (df['censor_date'] - df['eligibility_date']).dt.days
    df['death_rel_day'] = (df['DatumUmrti'] - df['eligibility_date']).dt.days

    # Weekly bins
    bins = np.arange(cfg.lag_min, cfg.lag_max + cfg.bin_size_days, cfg.bin_size_days)
    agg_list = []
    for bin_start in tqdm(bins, desc="Aggregating weekly bins"):
        bin_end = bin_start + cfg.bin_size_days
        # n_at_risk: number of subjects surviving to bin_start (censor_rel_day > bin_start)
        n_at_risk = (df['censor_rel_day'] > bin_start).sum()
        # n_deaths: deaths occurring in [bin_start, bin_end)
        n_deaths = ((df['death_rel_day'] >= bin_start) & (df['death_rel_day'] < bin_end)).sum()
        agg_list.append({
            'rel_bin': bin_start,
            'n_at_risk': n_at_risk,
            'n_deaths': n_deaths
    ...
    # Create dummy variables for event-study lags
    dummies = pd.get_dummies(agg['rel_bin'], prefix='lag').astype(float)
    agg = pd.concat([agg, dummies], axis=1)

    # Programmatic reference bin selection: closest pre-eligibility bin
    ...
    if not pre_bins.empty:
        ...
        log.info(f"Reference bin: {ref_bin} days (closest pre-0)")
    else:
        log.warning("No pre-0 bins for reference; no reference dropped")

    # Poisson GLM: deaths ~ lag dummies + offset(log(n_at_risk))
    ...
    # Pre-trends test: joint F-test on lead coefficients
    lead_cols = [c for c in X_cols if int(c.split('_')[-1]) < 0]
    pre_trends_p = np.nan
    if lead_cols:
        wald_test = model.wald_test(" = ".join(lead_cols) + " = 0")
        pre_trends_p = wald_test.pvalue
        log.info(f"Pre-trends joint F-test: p = {pre_trends_p:.4f}")

    # Extract coefficients and standard errors from modeled columns only
    coefs = model.params[X_cols]
    ses = model.bse[X_cols]
    times = [int(c.split('_')[-1]) for c in X_cols]

    # Safety check: ensure lengths match
    if len(times) != len(coefs):
        log.error(f"Length mismatch: times={len(times)}, coefs={len(coefs)}")
        raise ValueError("Length mismatch in coefs/times")
    ...
    fig.update_layout(
        title=f"Descriptive Event-Time Hazard Study: Mortality Timing Around Eligibility (Age {cfg.age_bins[0]}-{cfg.age_bins[1]-1})",
        xaxis_title=f"Days from Eligibility ({cfg.bin_size_days}-day bins)",
        yaxis_title="Log Hazard Ratio",
        hovermode="x unified"
    )
    #fig.write_html(cfg.out_dir / "event_study.html")
    
    fig.write_html(
    cfg.out_dir / "event_study.html",
    config={
    ...
    # Build results DataFrame: merge risk-set info by rel_bin to ensure alignment
    ...
    # Merge n_at_risk and n_deaths from agg by rel_bin
    ...
    # Save results
    results_df.to_csv(cfg.out_dir / "results.csv", index=False)

    log.info(f"Analysis complete. Interactive plot: {cfg.out_dir / 'event_study.html'}")

...
class Config:
    ...
    out_dir: Path = Path(r"C:\github\CzechFOI-DRATE-OPENSCI\Plot Results\AG_bias_necessity_rmst_AG_20_40")
    ...
    lag_min: int = -60
    lag_max: int = 120                        # Your requested full extended range

    vacc_quantiles: int = 12
    pre_vax_window_days: int = 60             # Matches negative lag range – strong power

    age_bins: tuple = (60, 80)                # ← CHANGE THIS FOR EACH RUN (see below)

    # ──────────────────────────────────────────────────────────────────────────────
    # PLACEBO & BOOTSTRAP – Balanced precision vs speed
    # ──────────────────────────────────────────────────────────────────────────────
    n_placebo_sims: int = 30                  # Good compromise: tighter than 30, faster than 50
    bootstrap_reps: int = 300                 # Excellent precision, ~33% faster than 300
    ...
    prop_at_tau_threshold: float = 0.3       # Very important! Prevents bootstrap hell in late lags
